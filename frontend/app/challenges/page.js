      ...
      <div className="flex-1 p-4 md:p-10">
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          <input
            ...
            className="p-3 rounded-xl bg-[#1e293b] border border-white/10"
          />
          <select
            value={difficulty}
          ...
          {challenges?.map((c, i) => (
            <div key={i} className="bg-white/10 p-6 rounded-3xl border border-white/10">
              <div className="flex justify-between items-start mb-3">
                <h3 className="text-2xl font-bold text-cyan-400">{c.title}</h3>
                <span className="text-sm bg-white/10 px-3 py-1 rounded-full">
                  {c.difficulty}
                </span>
              </div>
              <p className="text-gray-300 mb-4">{c.description}</p>
              <p className="text-gray-400 text-sm font-medium mb-2">Hints</p>
              <ul className="list-disc list-inside text-gray-300 mb-4">
                {c.hints?.map((h, j) => (
                  <li key={j}>{h}</li>
                ))}
              </ul>
              <p className="text-gray-400 text-sm font-medium mb-1">Solution outline</p>
              <p className="text-gray-200">{c.sample_solution_outline}</p>
            </div>
          ))}
        </div>
      </div>
